...
namespace Drupal\custom_misc\Plugin\Block;
...
class HeaderBlock extends BlockBase {
  ...
  public function build() {
    ...
    // Get search form
    //    $build['#content']['search'] = $this->_getSearchForm() ?: NULL;
    // Mobile lang switcher
    $build['#content']['language_switcher'] = $this->_get_language_switcher_block();
    $block = Block::load('mainnavigation');
    if ($block) {
      $build['#content']['main_menu'] = $block->getPlugin()->build();;
    }

//kint( $build['#content']['user_menu']);die;
    return $build;
  }
